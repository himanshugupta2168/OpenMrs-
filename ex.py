...
def fetch_patient_by_name(jsessionid, patient_name):
    ...
    if response.status_code == 200:
        ...
        print(f"Error fetching patient: {response.status_code}, {response.text}")
        return None
...
def list_dead_patients(jsessionid):
    ...
    if response.status_code == 200:
        ...
        death_info = {"dead_patients": []}
        
        for patient in patients:
            if patient.get("person", {}).get("dead"):
                death_info["dead_patients"].append({
                    "name": patient["display"],
                    "gender": patient["person"].get("gender", "Unknown"),
                    "deathDate": patient["person"].get("deathDate", "Unknown"),
                    "causeOfDeath": patient["person"].get("causeOfDeath", {}).get("display", "Unknown"),
                    "address": patient.get('preferredAddress', {}).get('display', 'Unknown')
                })
        return death_info
    else:
        print(f"Error fetching dead patients: {response.status_code}, {response.text}")
        return None
